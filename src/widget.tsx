...
import { MessageSquare, X, Mic, Send, Loader2, Bot, User, Minus, MicOff, FileText, Image as ImageIcon, File, Download, ImagePlus, MapPin, RotateCcw, Phone } from 'lucide-react';
...
const Widget = ({ agentId, companyId, backendUrl, rtlOverride, languageOverride, positionOverride, displayMode = 'widget' }: WidgetProps) => {
  ...
  const [voiceWorkflowSession, setVoiceWorkflowSession] = useState<{
    sessionId: string;
    roomName: string;
    livekitUrl: string;
    userToken: string;
  ...
  const connectVoiceWebSocket = async () => {
    ...
    voiceWs.current.onopen = async () => {
      console.log('[VoiceWS] ✅ Voice WebSocket OPENED', {
        url: voiceUrl,
        readyState: voiceWs.current?.readyState
      });
      ...
      if (settings?.communication_mode === 'voice' || settings?.communication_mode === 'chat_and_voice') {
        try {
          console.log('[VoiceWS] Requesting microphone permission...');
          const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
          audioStream.current = stream;
          console.log('[VoiceWS] ✅ Microphone stream acquired', {
            active: stream.active,
            tracks: stream.getTracks().map(t => ({ kind: t.kind, enabled: t.enabled, muted: t.muted }))
          });
        } catch (error) {
          console.error('[VoiceWS] ❌ Failed to get microphone permission:', error);
        }
      }
    };
    ...
    voiceWs.current.onclose = (event) => {
      console.log('[VoiceWS] 🔴 Voice WebSocket CLOSED', {
        code: event.code,
        reason: event.reason,
    ...
    voiceWs.current.onerror = (error) => {
      console.error('[VoiceWS] ❌ Voice WebSocket ERROR', error);
    };
  };
  ...
  const startVoiceActivityDetection = async () => {
    try {
      ...
      // Ensure AudioContext is running (needed for many browsers)
      ...
      mediaRecorder.current.ondataavailable = (e) => {
        if (e.data.size > 0) {
          ...
          console.debug(`[VAD] Collected chunk: ${e.data.size} bytes`);
        }
      };
      ...
      mediaRecorder.current.onstop = () => {
        if (audioChunks.current.length > 0 && voiceWs.current?.readyState === WebSocket.OPEN) {
          ...
          console.warn('[VAD] No audio data to send or WebSocket closed', {
            chunks: audioChunks.current.length,
            wsState: voiceWs.current?.readyState
          });
        }
        audioChunks.current = [];
      };

      // Start continuous recording with periodic triggers (500ms)
      mediaRecorder.current.start(500);
      console.log('[VAD] MediaRecorder STARTED', { state: mediaRecorder.current.state });
  ...
  const monitorAudioLevel = () => {
    ...
    const checkAudioLevel = () => {
      ...
      // Log energy if it's significant but below threshold (debug only)
      if (average > 10 && average <= SPEECH_THRESHOLD && !isSpeaking.current) {
        console.debug(`[VAD] Energy level: ${average.toFixed(1)} (Threshold: ${SPEECH_THRESHOLD})`);
      }

      if (average > SPEECH_THRESHOLD) {
        // User is speaking
        if (!isSpeaking.current) {
          console.log('[VAD] 🎙️ Speech detected, capturing audio...');
          isSpeaking.current = true;
          setIsVoiceActive(true);

          // Clear previous silence chunks so we only send the speech
          audioChunks.current = [];

          // If recorder is inactive for some reason, restart it
        ...
        if (isSpeaking.current) {
          if (silenceStart.current === null) {
            ...
            if (silenceDuration > SILENCE_DURATION) {
              console.log(`[VAD] 🤫 ${SILENCE_DURATION / 1000}s silence detected, finalized speech chunk`);
  ...
  const handleMicToggle = async () => {
    if (isMicEnabled) {
      // Turn mic off
      console.log('[Voice] 🔴 Mic OFF by user', {
        wasMicEnabled: isMicEnabled,
        wasVoiceActive: isVoiceActive,
        recorderState: mediaRecorder.current?.state
      });
      ...
      // Turn mic on
      console.log('[Voice] 🟢 Mic ON by user', {
        voiceWsState: voiceWs.current?.readyState,
        hasAudioStream: !!audioStream.current,
        streamActive: audioStream.current?.active
      });
  ...
  // Start Voice Workflow Call - fetches agent workflow and initiates LiveKit voice session
  const startVoiceWorkflowCall = async () => {
    try {
      setIsVoiceWorkflowLoading(true);
      console.log('[VoiceWorkflow] Starting voice workflow call...');

      // Step 1: Fetch the workflow assigned to this agent
      const workflowResponse = await fetch(`${backendUrl}/api/v1/agents/${agentId}/workflow`);
      if (!workflowResponse.ok) {
        throw new Error('Failed to fetch agent workflow');
      }
      const workflowData = await workflowResponse.json();
      console.log('[VoiceWorkflow] Fetched workflow:', workflowData?.name);

      // Step 2: Start voice workflow session
      const sessionResponse = await fetch(`${backendUrl}/api/v1/voice-workflow/start-session`, {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          user_name: 'Customer',
          greeting_message: localizedTexts?.welcome_message || settings?.welcome_message
        })
      });
      ...
      console.log('[VoiceWorkflow] Session started:', sessionData.session_id);

      // Store session data
      setVoiceWorkflowSession({
      ...
      setMessages(prev => [...prev, {
        id: `system-${Date.now()}`,
        sender: 'system',
        text: `Failed to start voice call: ${error instanceof Error ? error.message : 'Unknown error'}`,
        type: 'message',
        timestamp: new Date().toISOString()
      }]);
    } finally {
      setIsVoiceWorkflowLoading(false);
    }
  };
  ...
  const handleToggleRecording = async () => {
    ...
    if (isRecording) {
      console.log('[Mic] Stopping manual recording...');
      try {
      ...
      try {
        ...
        let stream = audioStream.current;
        if (!stream) {
          console.log('[Mic] Requesting fresh mic stream...');
          stream = await navigator.mediaDevices.getUserMedia({ audio: true });
          audioStream.current = stream;
        }
        ...
        if (!voiceWs.current || voiceWs.current.readyState !== WebSocket.OPEN) {
          console.error('[Mic] ❌ Voice WebSocket not open. Cannot record.');
          // Attempt to reconnect if missing
          if (settings?.communication_mode === 'voice' || settings?.communication_mode === 'chat_and_voice') {
            await connectVoiceWebSocket();
          } else {
            alert("Voice features are not enabled for this agent.");
            return;
          }
        }
        ...
        recorder.ondataavailable = e => {
          if (e.data.size > 0) {
            audioChunks.current.push(e.data);
            console.debug(`[Mic] Collected chunk: ${e.data.size} bytes (total chunks: ${audioChunks.current.length})`);
          }
        };
        ...
        recorder.onstop = () => {
          ...
          if (audioChunks.current.length > 0 && voiceWs.current?.readyState === WebSocket.OPEN) {
            ...
            console.warn('[Mic] Recording stopped but nothing to send', {
              chunks: audioChunks.current.length,
              wsState: voiceWs.current?.readyState
            });
          }
        ...
        recorder.start(500); // Trigger data every 500ms
      } catch (err) {
        console.error("[Mic] ❌ Mic access error:", err);
        setIsRecording(false);
      }
    }
  };
